...
local function command_succeeded(result, how, code)
        ...
        if chapter_response.type == "SUCCESS" then
            for _, chapter in ipairs(chapter_response.body) do
                if chapter.downloaded then
                    ...
                    local target = downloads_path .. "/" .. chapter_filename(manga, chapter)
                    if current ~= target and exists(current) and not exists(target) then
                        plan[#plan + 1] = {
                            current = current,
                            target = target,
                        }
...
function RakuyomiRenamer:scanAndConfirm()
    ...
    if #plan.items == 0 then
        show_message(_("No Rakuyomi downloads need renaming."))
        return
    end
    UIManager:show(ConfirmBox:new({
        text = _("Rename ") .. #plan.items .. _(" downloaded chapter file(s)?\n\n") ..
            _("This moves each real CBZ to a friendly name. It will try to leave a tiny compatibility link for Rakuyomi, but some Kindle storage folders do not support links.\n\nFolder:\n") ..
            plan.downloads_path,
        ok_text = _("Rename"),
...
function RakuyomiRenamer:runRenamePlan(plan)
    local renamed = 0
    local failed = 0
    local compatibility_links = 0
    local compatibility_failed = 0

    for _, item in ipairs(plan.items) do
        local ok, rename_err = os.rename(item.current, item.target)
        if ok then
            renamed = renamed + 1

            local symlink_result, symlink_how, symlink_code =
                os.execute("ln -s " .. shell_quote(item.target) .. " " .. shell_quote(item.current))
            if command_succeeded(symlink_result, symlink_how, symlink_code) then
                compatibility_links = compatibility_links + 1
            else
                local hardlink_result, hardlink_how, hardlink_code =
                    os.execute("ln " .. shell_quote(item.target) .. " " .. shell_quote(item.current))
                if command_succeeded(hardlink_result, hardlink_how, hardlink_code) then
                    compatibility_links = compatibility_links + 1
                else
                    compatibility_failed = compatibility_failed + 1
                    logger.warn(
                        "Rakuyomi Renamer: renamed but could not create a symlink or hardlink for ",
                        item.target
                    )
                end
            end
            ...
            failed = failed + 1
            logger.warn("Rakuyomi Renamer: could not rename ", item.current, " to ", item.target, ": ", rename_err)
        end
    end

    show_message(
        _("Renamed ") .. renamed ..
        _(" file(s). Compatibility links: ") .. compatibility_links ..
        _(". Link failures: ") .. compatibility_failed ..
        _(". Rename failures: ") .. failed ..
        _(". Skipped: ") .. plan.skipped .. "."
    )
end
